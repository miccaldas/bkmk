...
def show_results():
    ...
    """
    cnf = tput_config()
    bkmkfldr = "/home/mic/python/bkmk/bkmk"

    entries = []
    with open(f"{bkmkfldr}/srch.bin", "rb") as f:
        while True:
            try:
    ...
    lns_val = [(len(a), len(b), len(c), len(d), len(e), len(f), len(g), len(h)) for a, b, c, d, e, f, g, h in vals]
    ...
    with open(f"{bkmkfldr}/see_results.bash", "w") as f:
        ...
        table_width = int(cnf["init_width"] + (len(cnf["separator"]) / 2) - (len(f"{ntraces}") / 2))
        ...
        for r in rng_lst:
            ...
            if r == 0:
                ...
                f.write(f"echo '| {values[r][1][0]}{space_title_n0} | {values[r][1][1]}{space_title_v0} |'\n")
                f.write(f"tput cup {first_item_height + 2} {table_width}\n")
                f.write(f"echo '| {values[r][2][0]}{space_comment_n0} | {values[r][2][1]}{space_comment_v0} |'\n")
                f.write(f"tput cup {first_item_height + 3} {table_width}\n")
                f.write(f"echo '| {values[r][3][0]}{space_link_n0} | {values[r][3][1]}{space_link_v0} |'\n")
                f.write(f"tput cup {first_item_height + 4} {table_width}\n")
                f.write(f"echo '| {values[r][4][0]}{space_k1_n0} | {values[r][4][1]}{space_k1_v0} |'\n")
                f.write(f"tput cup {first_item_height + 5} {table_width}\n")
                f.write(f"echo '| {values[r][5][0]}{space_k2_n0} | {values[r][5][1]}{space_k2_v0} |'\n")
                f.write(f"tput cup {first_item_height + 6} {table_width}\n")
                f.write(f"echo '| {values[r][6][0]}{space_k3_n0} | {values[r][6][1]}{space_k3_v0} |'\n")
                f.write(f"tput cup {first_item_height + 7} {table_width}\n")
                f.write(f"echo '| {values[r][7][0]}{space_tempo_n0} | {values[r][7][1]}{space_tempo_v0} |'\n")
                ...
                f.write(f"echo '| {values[r][7][0]}{space_tempo_n0} | {values[r][7][1]}{space_tempo_v0} |'\n")
                f.write(f"tput cup {first_item_height + 9 * r + 10} {table_width}\n")
                f.write(f"echo '{ntraces}'\n")
                f.write(f"tput cup {first_item_height + 9 * r + 11} {table_width}\n")
                f.write("echo ' '\n")

        f.write(f"tput cup {12 * len(rng_lst) + 13} {table_width}\n")
    ...
    subprocess.run(f"sudo chmod +x {bkmkfldr}/see_results.bash", shell=True)
    subprocess.run(f"{bkmkfldr}/see_results.bash", shell=True)
...
def db_call(srch):
    ...
    os.remove(f"{bkmkfldr}/srch.bin")
    os.remove(f"{bkmkfldr}/see_results.bash")
